...
st.title("EQUISIMA - V13 FINAL CON FIRMAS")
...
with st.sidebar:
    ...
    responsable=st.text_input("Responsable de la Medicion","edwin cortes")
...
foto=st.file_uploader("FOTO ESQUEMA",type=['jpg','jpeg','png'])
...
def build_excel():
    ...
    ws['A5']="CLIENTE:"; ws['A5'].font=bold; ws['A5'].border=border
    ws['B5']=cliente; ws['B5'].border=border
    ws.merge_cells('B5:E5')
    ws['I5']="DATOS DEL EQUIPO UTILIZADO"; ws['I5'].font=bold; ws['I5'].border=border

    ws['A6']="NOMBRE DEL PROYECTO:"; ws['A6'].font=bold; ws['A6'].border=border
    ws['B6']=proyecto; ws['B6'].border=border
    ws.merge_cells('B6:E6')
    ws['I6']="EQUIPO"; ws['I6'].font=bold; ws['I6'].border=border
    ws['K6']="MARCA"; ws['K6'].font=bold; ws['K6'].border=border

    ws['A7']="DEPARTAMENTO :"; ws['A7'].font=bold; ws['A7'].border=border
    ws['B7']=depto; ws['B7'].border=border
    ws.merge_cells('B7:E7')
    ws['I7']="SONOMETRO"; ws['I7'].border=border
    ws['K7']=marca_sono; ws['K7'].border=border

    ws['A8']="MUNICIPIO:"; ws['A8'].font=bold; ws['A8'].border=border
    ws['B8']=muni; ws['B8'].border=border
    ws.merge_cells('B8:E8')
    ws['I8']="PISTOFONO"; ws['I8'].border=border
    ws['K8']="HD2020"; ws['K8'].border=border
    ...
    for p in st.session_state.puntos:
        ...
        ws.cell(row=r, column=5, value=p[9]).border=border
        ws.cell(row=r, column=6, value=p[10]).border=border
    ...
    # --- PIE QUE FALTABA - RESPONSABLE Y FIRMAS ---
    r+=2
    ws.cell(row=r, column=1, value="Responsable de la Medicion:").font=Font(bold=True,size=11)
    ws.cell(row=r, column=6, value=responsable).font=Font(bold=True,size=11)
    # linea
    ws.merge_cells(start_row=r, start_column=6, end_row=r, end_column=14)
    for cc in range(6,15):
        ws.cell(row=r, column=cc).border=Border(bottom=Side(style='thin'))
    r+=2

    # Tabla de firmas
    # Fila 1: Nombre y firma x3
    ...
    for cc in [1,6,10]:
        c=ws.cell(row=r, column=cc, value="Nombre y firma")
        c.font=bold8; c.alignment=Alignment(horizontal='center')
        c.border=border
    for cc in range(1,15):
        ws.cell(row=r, column=cc).border=border
    r+=1

    # Fila 2: Elaboro / Reviso / Aprobo
    ws.merge_cells(start_row=r, start_column=1, end_row=r, end_column=5)
    ws.merge_cells(start_row=r, start_column=6, end_row=r, end_column=9)
    ws.merge_cells(start_row=r, start_column=10, end_row=r, end_column=14)
    ws.cell(row=r, column=1, value="Elaboro:").alignment=Alignment(horizontal='center')
    ws.cell(row=r, column=6, value="Reviso:").alignment=Alignment(horizontal='center')
    ws.cell(row=r, column=10, value="Aprobo: Gerente G.").alignment=Alignment(horizontal='center')
    for cc in range(1,15):
        ws.cell(row=r, column=cc).border=border
        ws.cell(row=r, column=cc).font=Font(size=8)
    r+=1
    ...
    ws.merge_cells(start_row=r, start_column=6, end_row=r, end_column=9)
    ws.merge_cells(start_row=r, start_column=10, end_row=r, end_column=14)
    ws.cell(row=r, column=1, value="Fecha:").alignment=Alignment(horizontal='center')
    ws.cell(row=r, column=6, value="Fecha:").alignment=Alignment(horizontal='center')
    ws.cell(row=r, column=10, value="Fecha:").alignment=Alignment(horizontal='center')
    for cc in range(1,15):
        ws.cell(row=r, column=cc).border=border
        ws.cell(row=r, column=cc).font=Font(size=8)
...
if st.session_state.puntos:
    excel=build_excel()
    st.download_button("DESCARGAR EXCEL FINAL CON FIRMAS", excel, f"R2-POE37-EP_{muni}_FINAL_FIRMAS.xlsx", type="primary", use_container_width=True)
